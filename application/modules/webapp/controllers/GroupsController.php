...
class Webapp_GroupsController extends Zend_Controller_Action {
    ...
    public function indexAction() {
        ...
        $phoneGroups = $groupMapper->fetchList('phone_id=' . $phoneId, 'type ASC');
        $contactMapper = new Sos_Model_ContactMapper();
        $groups = array();
        foreach($phoneGroups as $g) {
            $contactGroupCount = $contactMapper->countContactsByPhoneId($phoneId, '', $g->getId());
            $groupArr = array();
            $groupArr['id'] = $g->getId();
            $groupArr['name'] = htmlspecialchars($g->getName());
            $groupArr['type'] = $g->getType();
            $groupArr['phoneId'] = $g->getPhoneId();
            $groupArr['contactCount'] = $contactGroupCount;
            $groups[] = $groupArr;
        }  
    ...
    public function contactsAction() {
        ...
        if ($groupId && $group->getId() && ($group->getPhoneId() == $phone->getId())) {
            ...
            Zend_Layout::getMvcInstance()->assign('title', htmlspecialchars($group->getName()));
            $contacts = $contact->fetchList('group_id=' . $groupId, 'type DESC');
        } else {
