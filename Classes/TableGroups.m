...
#define kGroupNameTextFieldTag  999
...
//	NSLog(@"row = %d",buttonRow);
    ...
    if (row == 2) {
        ...
        textField.tag = kGroupNameTextFieldTag;        
    }
    if (row > 2) {
...
- (BOOL)textField:(UITextField *)_textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string {
    NSString* proposedString = [_textField.text stringByReplacingCharactersInRange:range withString:string];
    if ([proposedString length] <= 10)
        return YES;
    return NO;
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	if(indexPath.section==0)
	{
        ...
        button.tag = [groupP.idGroup integerValue];
        ...
        if ([groupP isAllowEditName]) cell.accessoryView = button;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    tbGroup.hasJoinFeature = NO;
    if (indexPath.row == 2) {
        tbGroup.hasJoinFeature = YES;
    }
	...
	if ( flag == 1) 
	{
        ...
        //	[arrayGroup removeAllObjects];
            for(NSDictionary *dict in data)
            {
                ...
                groupPersonal.allowEditName = [[dict objectForKey:@"allowEdit"] isEqual:@"true"] ? YES : NO;
                groupPersonal.allowDelete = [[dict objectForKey:@"allowDelete"] isEqual:@"true"] ? YES : NO;
