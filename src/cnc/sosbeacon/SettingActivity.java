...
public class SettingActivity  extends GeneralActivity {
	...
	protected void saveSettingsConfirm(final Handler handler, Boolean finishActivity, final Boolean updateAccount, Boolean confirm ) {
		if (finishActivity) {
			if (confirm) {
				mAlertDialog = new AlertDialog.Builder(this);
				mAlertDialog.setMessage(R.string.setting_save_confirm);
				mAlertDialog.setNeutralButton(R.string.btnYes, new DialogInterface.OnClickListener() {
					public void onClick(DialogInterface dialog, int which) {
						updateSetting(handler, updateAccount);
					}
				});
				mAlertDialog.setNegativeButton(R.string.btnNo, new DialogInterface.OnClickListener() {
					public void onClick(DialogInterface dialog, int which) {
	...
	protected void updateSetting(final Handler newHandler, final Boolean updateAccount) {
		mProgressDialog = ProgressDialog.show(SettingActivity.this, "", getString(R.string.saving), true);
		new Thread(new Runnable() {
			public void run() {
				if (updateAccount) {
					updatePhone(UPDATE, mUserName, mEmail, mPassword);
				} else {
					sendSetting();
				}
	...
	private void sendSetting() {
		try {	
			...
			httpObject.put(ALERT_SEND_TO_GROUP, mAlertSendToGroup);
			...
			try {
				...
				if (state) {
					initPhoneSettings(responseJson.getJSONObject(DATA));
					savePhoneInfor();
				}
