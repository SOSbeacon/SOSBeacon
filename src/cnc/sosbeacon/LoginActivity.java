...
public class LoginActivity extends GeneralActivity {
	...
	private void processLogin() {
		...
		if (mResponseCode == CODE_ACCOUNT_NEW_IMEI || mResponseCode == CODE_ACCOUNT_NEW_NUMBER) {
			final AlertDialog adAction = new AlertDialog.Builder(this).setMessage(mMessage).create();
    ...
    private CountDownTimer waitingCountDown = new CountDownTimer(WAITING_ACTIVATE_TIME, 1000) {
		...
		public void onFinish() {
			...
			if (waitingCount == 1) { // Login again in first waiting
				...
				ackReceived.setNeutralButton(getString(R.string.btnOK), new DialogInterface.OnClickListener() {
					public void onClick(DialogInterface dialog, int which) {
						login();
					}
				});
				ackReceived.setNegativeButton(getString(R.string.btnNo), new DialogInterface.OnClickListener() {
					public void onClick(DialogInterface dialog, int which) {
						etNumber.requestFocus();
						etNumber.setText(mPhoneNumber);
						new AlertDialog.Builder(LoginActivity.this)
						.setMessage(R.string.verifyPhonePrompt).setNegativeButton(R.string.btnOK, null).show();
						loginForm.setVisibility(View.VISIBLE);
					}
				});
				ackReceived.show();
	...
	private void forgotPasswordPrompt() {
		mHandler = new Handler() {
			...
			public void handleMessage(Message msg) {
				...
				mProgressDialog.hide();
				new AlertDialog.Builder(LoginActivity.this).
				setMessage(mMessage).setNeutralButton(R.string.btnOK, null).show();
			}
		};
