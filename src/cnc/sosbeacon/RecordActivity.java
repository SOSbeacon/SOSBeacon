...
import cnc.sosbeacon.recorder.AudioRecorder;
...
public class RecordActivity extends GeneralActivity implements SurfaceHolder.Callback,Runnable, Camera.PreviewCallback {
	...
	/*
	 * Addition (non-Javadoc)
	 * @see cnc.sosbeacon.GeneralActivity#onCreate(android.os.Bundle)
	 */
	MediaPlayer player;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		returnActivity = getIntent().getStringExtra(ACTIVITY) != null ? getIntent().getStringExtra(ACTIVITY) : "";
		alertId = getIntent().getStringExtra(ALERT_ID);
		sendToGroupId = getIntent().getIntExtra(GROUP_ID, 0);
		checkInMessage = getIntent().getStringExtra(MESSAGE);
		singleContact = getIntent().getStringExtra(SINGLE_CONTACT);
		// alertLogType = check in: 2, alert: 0, call: 1
		if (returnActivity.equalsIgnoreCase("CheckInActivity")) {
			type = "2";
		}
		...
		callBackHandler = new Handler() {
			...
			public void handleMessage(Message msg) {
				...
				finish();		
				logger.log(Level.INFO, "callBackHandler, checkInResultMessage" + checkInResultMessage);
				if (returnActivity.equalsIgnoreCase("NeedHelpActivity")) {
					NeedHelpActivity.show(RecordActivity.this, true);
				}
				if (returnActivity.equalsIgnoreCase("CheckInActivity")) {
					CheckInActivity.show(RecordActivity.this, checkInResultMessage);
				}
			}
		};
	...
	/*private void takePicture() {
		if (!onPictureProcessing) {
			onPictureProcessing = true;
			btnCapture.setEnabled(false);
			btnCapture.setVisibility(View.INVISIBLE);
			//logger.log(Level.INFO, "===== takePicture");
			if (_camera != null && !isRecordStopped) {
				//btnCapture.setBackgroundResource(R.drawable.camera);
				try {
					Surface.setOrientation(Display.DEFAULT_DISPLAY, SensorManager.SENSOR_ORIENTATION);
					_camera.setPreviewDisplay(_surfaceHolder);
					setCameraParams();
				} catch (IOException e) {
					logger.log(Level.ERROR, "takePicture: " + e.getMessage());
				}			
				playSound();
				try {
					_camera.takePicture(_shutterCallback, _rawCallback, _jpgCallback);
					numberPhoto++;
				} catch (Exception e) {
					logger.log(Level.ERROR, "takePicture: " + e.getMessage());
				}
			...
			_jpgCallback = new Camera.PictureCallback() {
				public void onPictureTaken(byte[] data, Camera camera) {
					...
					try {
						...
						logger.log(Level.ERROR, "addCameraCallback: _camera.startPreview: " + e.getMessage());
					}
	...
	public static void show(Context context, String returnActivity, String alertid) {
		show(context, returnActivity, alertid, 0, "", "");
	}
	
	public static void show(Context context, String returnActivity,
			String alertId, Integer sendToGroupId, String singleContact,
			String message) {
		final Intent intent = new Intent(context, RecordActivity.class);
		intent.putExtra(ACTIVITY, returnActivity);
		intent.putExtra(ALERT_ID, alertId);
		intent.putExtra(GROUP_ID, sendToGroupId);
		intent.putExtra(SINGLE_CONTACT, singleContact);
	...
	private void setCameraParams() {
		...
		if (!isRecordStopped) {
			try {
				...
				if (sdkVersion >= 8) {
					...
					//cnc.sosbeacon.util.CameraApi8.setCameraOrientation(_camera, 90);
					//p.setRotation(90);
				} else{
		...
		//logger.log(Level.INFO, "===== END setCameraParams() =====");
	}
	
	private void recordProgress() {
		//logger.log(Level.INFO, "===== START recordProgress, count=" + count + ", isRecordStopped=" + isRecordStopped + ", isManualCapture=" + isManualCapture);
		...
		if(!isRecordStopped) {
			...
			if (!isManualCapture) {
				...
				if ((remainderCount > 10) && (remainderCount < recordDuration) && ((captureTimeDown == 1) || (captureTimeDown == 2) || (captureTimeDown == 3))) {
				...
				if ((remainderCount >= 10) && (remainderCount < recordDuration) && (captureTimeDown == 0)) {
					txtCameraCountdown.setVisibility(View.INVISIBLE);
					capture = true;
					//takePicture();
				}
		...
		//logger.log(Level.INFO, "===== END recordProgress");
	}
			...
			if (type.equalsIgnoreCase("2")) {
				Handler checkinHandler = new Handler() {
					...
					public void handleMessage(Message msg) {
						...
						if (msg.arg1 > 0 ) {
							alertId = String.valueOf(msg.arg1);
							if (lstFileImage.size() > 0 || lstFileVoice.size() > 0) {
								sendServer();
							} else {
								callBackHandler.sendEmptyMessage(0);
				...
				sendCheckIn(checkinHandler, sendToGroupId, singleContact, checkInMessage);
			}
		}
	}
	...
	private void sendServer() {
		...
		try {
			UploadDialog uploadDialog = new UploadDialog(RecordActivity.this,
					getApiUrl(UPLOAD_URL), callBackHandler, lstFileImage,
					lstFileVoice, alertId, type, getPhoneId(), getToken());
	...
	private boolean checkSdCard() {
		...
		if (!state.equals(android.os.Environment.MEDIA_MOUNTED)) {
			Toast.makeText(this, "SD Card is not mounted.  It is " + state + ".", Toast.LENGTH_SHORT).show();
			return false;
		}
		return true;
	}
	...
	private void createDirectory(String nDirectory) {
		try {
			String path = Environment.getExternalStorageDirectory().getAbsolutePath() + "/" + nDirectory;
				...
				public void run() {
					...
					try {
				        ...
				        File file = new File(Environment.getExternalStorageDirectory()
				                .getPath() + "/SOSbeacon/" +String.format("%d", time)+".jpg");
