...
import cnc.sosbeacon.http.HttpUploadFile;
...
public class UploadDialog extends Dialog implements Constants {
	...
	public UploadDialog(Context context, String uploadUrl, Handler callBackHandler, List<String> lstFileImage,List<String> lstFileVoice, String alertId, String type, String phoneId, String token) {
		super(context);
	...
	private void setUploadDetail(Message msg, int type, boolean isFinish) {
		...
		//String uploadAuidoDetail = "\nUploading audio %d \nSuccessful: %d";
		//String uploadAudioDetailFails = ", failed: %d";
		
		String uploadImagesSuccess = "Upload photos done!";
		//String uploadImagesFails = ", %d image(s) failed";
		String uploadAudioSuccess = "\nUpload audio done!";
		String uploadAudioFails = "\nUpload audio failed";
		
		String details = "";
		if (isFinish) {
			//if (countImage > 0) details = String.format(uploadImagesSuccess, imageSuccess);
			//if (imageFail > 0) details = String.format(uploadImagesSuccess + uploadImagesFails, imageSuccess, imageFail);
			//if (audioSuccess > 0) details += String.format(uploadAudioSuccess, audioSuccess);
			//if (audioFail > 0) details += String.format(uploadAudioFails, audioFail);
			if (countImage > 0) details = uploadImagesSuccess;
			details += uploadAudioSuccess;
		} else {
			if (type == 0) {
				if (countImage > 0) details = String.format(uploadImageDetail, countImage, imageSuccess);
				if (imageFail > 0) details = String.format(uploadImageDetail + uploadImageDetailFails, countImage, imageSuccess, imageFail);	
			} else {
				if (countImage > 0) details = uploadImagesSuccess;
				if (audioFail > 0) {
		...
		if (msg != null) if (msg.obj != null) Toast.makeText(context, msg.obj.toString(), Toast.LENGTH_SHORT).show();
	}
	...
	private void upload() {
				...
				public void handleMessage(Message msg) {
				 switch (msg.what) {
					...
					case 10:
						...
						try {
							if(UploadDialog.this.isShowing()) UploadDialog.this.dismiss();
						} catch (Exception e) {
							e.printStackTrace();
						}
		 ...
		 // alertLogType = check in: 2, alert: 0, call: 1
		 Thread workthreadImage = new Thread(new HttpUploadFile(uploadUrl, uHandler, lstFileImage,lstFileVoice , alertId, type, phoneId, token));
